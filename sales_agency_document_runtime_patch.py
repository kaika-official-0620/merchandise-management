...
def prepare(module: Any) -> None:
    ...
    module._sales_agency_save_vendor_response_file = save_vendor_response_file
...
def apply(module: Any) -> None:
    ...
    save_vendor_response_file = module._sales_agency_save_vendor_response_file
    ...
    def is_admin_generated_invoice(row: dict | None) -> bool:
        ...
        return (bool(row.get("source_admin_kaitori_id")) or bool(row.get("sales_agency_request_id"))
                or bool(row.get("created_by_admin_id")) or not module.is_user_created_invoice_document(row))
    ...
    def is_user_visible_invoice(row: dict | None) -> bool:
        ...
        if not is_admin_generated_invoice(row):
            return True
        return not module.is_deletable_document_status(row.get("status"))

    def is_admin_vendor_mitsumori(row: dict | None) -> bool:
        if not row:
            return False
        return (row.get("document_scope") or "").strip() in {"vendor_outgoing", "kaika_vendor_outgoing", "kaika_estimate"}
    ...
    def user_invoice_list_v2():
        ...
        try:
            cur.execute(
                ...
                SELECT * FROM invoices
                WHERE sender_id = {mark()}
                  AND COALESCE(document_scope, 'client_outgoing') IN ('client_outgoing', 'user_created')
                ORDER BY created_at DESC
                """,
                (current_user.id,),
            )
            invoices = [invoice for invoice in rows_to_dicts(cur.fetchall()) if is_user_visible_invoice(invoice)]
        finally:
            cur.close()
            conn.close()
    ...
    @login_required
    def invoice_download_v2(id: int):
        invoice = (load_invoice_for_user(id) or [None])[0]
        if not current_user.is_admin() and invoice and not is_user_visible_invoice(invoice):
            flash("この買取明細書は開花側で確認中のため、まだ表示できません。", "error")
            return redirect(url_for("user_invoice_list"))
        return originals["invoice_download"](id)

    @login_required
    def invoice_pdf_v2(id: int):
        invoice = (load_invoice_for_user(id) or [None])[0]
        if not current_user.is_admin() and invoice and not is_user_visible_invoice(invoice):
            flash("この買取明細書は開花側で確認中のため、まだ表示できません。", "error")
            return redirect(url_for("user_invoice_list"))
        return originals["invoice_pdf"](id)

    @login_required
    def user_mitsumori_download_v2(id: int):
        mitsumori = (load_mitsumori_for_user(id) or [None])[0]
        if not current_user.is_admin() and mitsumori and not is_visible_user_scope(mitsumori, "client_incoming"):
            flash("この見積依頼書はユーザー向けの書類ではありません。", "error")
            return redirect(url_for("user_mitsumori_list"))
        return originals["user_mitsumori_download"](id)

    @login_required
    def user_mitsumori_pdf_v2(id: int):
        mitsumori = (load_mitsumori_for_user(id) or [None])[0]
        if not current_user.is_admin() and mitsumori and not is_visible_user_scope(mitsumori, "client_incoming"):
            flash("この見積依頼書はユーザー向けの書類ではありません。", "error")
            return redirect(url_for("user_mitsumori_list"))
        return originals["user_mitsumori_pdf"](id)
    ...
    # Later document patches use this predicate too, so export the same rule
    # used by the list, detail, CSV and PDF handlers.
    module.is_user_visible_invoice = is_user_visible_invoice
    module.user_invoice_list = user_invoice_list_v2
    module.user_invoice_view = user_invoice_view_v2
    module.invoice_download = invoice_download_v2
    module.invoice_pdf = invoice_pdf_v2
    module.user_mitsumori_download = user_mitsumori_download_v2
    module.user_mitsumori_pdf = user_mitsumori_pdf_v2
    ...
    app.view_functions["invoice_download"] = invoice_download_v2
    app.view_functions["invoice_pdf"] = invoice_pdf_v2
    app.view_functions["user_mitsumori_download"] = user_mitsumori_download_v2
    app.view_functions["user_mitsumori_pdf"] = user_mitsumori_pdf_v2
