...
from invoice_tax import TAX_CATEGORIES, inclusive_breakdown, invoice_tax_context, invoice_tax_category_label, normalize_tax_category
...
def apply(module: Any) -> None:
    ...
    app = module.app
    app.jinja_env.globals.update(invoice_tax_context=invoice_tax_context, invoice_tax_category_label=invoice_tax_category_label)
    ...
    def ensure_step4_schema() -> None:
        ...
        try:
            invoice_columns = [
                ...
                ("tax_breakdown_version", "ALTER TABLE invoices ADD COLUMN tax_breakdown_version INTEGER DEFAULT 0", "ALTER TABLE invoices ADD COLUMN tax_breakdown_version INTEGER DEFAULT 0"),
                ("tax_rounding_method", "ALTER TABLE invoices ADD COLUMN tax_rounding_method VARCHAR(20)", "ALTER TABLE invoices ADD COLUMN tax_rounding_method TEXT"),
            ]
            item_columns = [
    ...
    def _insert_invoice(user_id: int, user_row: dict[str, Any], selected_items: list[dict[str, Any]], form_items: list[dict[str, Any]]) -> int:
        ...
        tax_breakdown = inclusive_breakdown(form_items)
        ...
        try:
            ...
            cur.execute(
                f"UPDATE invoices SET source_admin_kaitori_id = {mark()}, tax_breakdown_version=1, tax_rounding_method='floor' WHERE id = {mark()}",
    ...
    def admin_step4_client_invoice_create(user_id: int):
        ...
        if request.method == "POST":
            ...
            for item in selected_items:
                ...
                try:
                    tax_category = normalize_tax_category(request.form.get(f"tax_category_{item_id}"))
                    if client_amount < 0 or vendor_amount < 0:
