...
def apply(module: Any) -> None:
    ...
    def load_item_options(user_id: int | None = None, limit: int | None = 300, scope: str = "user") -> list[dict[str, Any]]:
        conn, cur = open_cursor()
        try:
            ph = placeholder()
            params: list[Any] = []
            ...
            if scope == "kaika":
                ...
                where_parts.append("m.sale_date IS NULL")
            elif scope == "user":
            ...
            cur.execute(
                ...
                SELECT m.id, m.product_name, m.brand_name, m.user_id, m.kaika_product_code,
                       m.photo_path, m.purchase_price, m.listing_price, m.model_number,
                       m.created_at, m.sale_date, m.is_shipped, m.item_condition,
                       m.appraisal_status, m.is_listed,
            ...
            for item in items:
                ...
                if item.get("sale_date"):
                    item["current_status_label"] = "売却済み"
                elif item.get("is_shipped"):
                    item["current_status_label"] = "発送済み"
                elif item.get("appraisal_status") == "waiting":
                    item["current_status_label"] = "査定待ち"
                elif item.get("appraisal_status") == "inspecting":
                    item["current_status_label"] = "査定中"
                elif item.get("is_listed"):
                    item["current_status_label"] = "出品中"
                else:
                    item["current_status_label"] = "未出品"
            return items
        finally:
            cur.close()
            conn.close()

    def is_selectable_kaika_item(item_id: int | None, cursor=None, lock_row: bool = False) -> bool:
        if not item_id:
            return False
        ...
        if owns_cursor:
            conn, cur = open_cursor()
        try:
            lock_clause = " FOR UPDATE OF m" if DATABASE_URL and lock_row else ""
            cur.execute(
                f"""
                SELECT m.id
                FROM merchandise m
    ...
    def default_monthly_fee_for_user(user_id: int) -> int:
        ...
        count = safe_int(
            fetch_scalar(
                f"SELECT COUNT(*) FROM merchandise WHERE user_id = {placeholder()} AND COALESCE(custody_location, 'kaika') = 'kaika'",
                (user_id,),
            )
        )
        if count == 0:
            has_legacy_contract = safe_int(fetch_scalar(
                f"""SELECT COUNT(*) FROM users WHERE id = {placeholder()}
                    AND (COALESCE(stripe_subscription_id, '') <> ''
                         OR subscription_status IN ('active', 'past_due', 'canceling'))""",
                (user_id,),
            ))
            if not has_legacy_contract:
                return 0
        try:
    ...
    def admin_vendor_documents():
        ...
        items = load_item_options(
            None if is_kaika_scope else selected_user_id,
            limit=None if is_kaika_scope else 300,
            scope="kaika" if is_kaika_scope else "user",
        )
        if request.method == "POST":
            ...
            try:
                ...
                raw_item_id = (raw_item_ids[0] if raw_item_ids else "") or ""
                ...
                if is_kaika_scope and (len(raw_item_ids) != 1 or not raw_item_id):
                    raise ValueError("開花商品を商品選択欄から1件選択してください。")
                if raw_item_id and (item_id is None or item_id <= 0):
                    raise ValueError("選択した商品IDが正しくありません。商品を再選択してください。")
                if is_kaika_scope and not is_selectable_kaika_item(item_id):
                    raise ValueError("選択した商品は開花商品でないか、現在選択できません。")
                ...
                try:
                    ...
                    if is_kaika_scope and not is_selectable_kaika_item(item_id, cursor=cur, lock_row=True):
                        raise ValueError("選択した商品は開花商品でないか、現在選択できません。")
                    cur.execute(
                        f"""
                ...
                if stored_path and not document_saved:
                    upload_root = os.path.realpath(os.path.join(app.static_folder, "uploads", "vendor_documents"))
                    upload_path = os.path.realpath(os.path.join(app.static_folder, *stored_path.split("/")))
                    try:
                        if os.path.commonpath([upload_root, upload_path]) == upload_root and os.path.isfile(upload_path):
