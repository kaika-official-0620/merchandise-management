...
from kaika_fee_counts import is_kaika_fee_item, is_kaika_fee_client
...
def apply(module: Any) -> None:
    ...
    save_validated_product_image = getattr(module, "save_validated_product_image", None)
    verify_google_drive_image_receipt = getattr(module, "verify_google_drive_image_receipt", None)
    remove_uploaded_relative_paths = getattr(module, "remove_uploaded_relative_paths", None)
    ProductImageValidationError = getattr(module, "ProductImageValidationError", ValueError)
    ...
    def save_uploaded_image(file_storage, prefix=""):
        if not file_storage or not getattr(file_storage, "filename", ""):
            return None
        if callable(save_validated_product_image):
            return save_validated_product_image(file_storage, prefix=prefix)
    ...
    def reject_legacy_drive_paths():
        if request.form.get("google_drive_photo_path") or request.form.get("google_drive_additional_paths"):
            raise ProductImageValidationError(
                "保存済み画像パスは指定できません。Google Driveから画像を選び直してください。"
            )

    def collect_verified_drive_uploads(is_kaika_scope):
        """署名済みDrive画像を検証し、通常multipartと分離して返す。"""
        drive_main = request.files.get("google_drive_photo_file")
        drive_main_receipt = request.form.get("google_drive_photo_receipt") or ""
        drive_additional = request.files.getlist("google_drive_additional_files")
        drive_additional_receipts = request.form.getlist("google_drive_additional_receipts")
        has_drive_payload = bool(
            (drive_main and getattr(drive_main, "filename", ""))
            or drive_main_receipt
            or any(getattr(file_storage, "filename", "") for file_storage in drive_additional)
            or drive_additional_receipts
        )
        if has_drive_payload and not is_kaika_scope:
            raise ProductImageValidationError("Google Drive画像は開花商品だけに使用できます。")
        if not has_drive_payload:
            return None, []
        if not callable(verify_google_drive_image_receipt):
            raise ProductImageValidationError("Google Drive画像を検証できません。")

        verified_ids = set()
        if bool(drive_main and getattr(drive_main, "filename", "")) != bool(drive_main_receipt):
            raise ProductImageValidationError("メインDrive画像の確認情報が一致しません。")
        if len(drive_additional) != len(drive_additional_receipts):
            raise ProductImageValidationError("追加Drive画像の確認情報が一致しません。")

        if drive_main:
            payload = verify_google_drive_image_receipt(drive_main, drive_main_receipt)
            verified_ids.add(payload["file_id"])
        for file_storage, receipt in zip(drive_additional, drive_additional_receipts):
            payload = verify_google_drive_image_receipt(file_storage, receipt)
            if payload["file_id"] in verified_ids:
                raise ProductImageValidationError("同じGoogle Drive画像は重複して追加できません。")
            verified_ids.add(payload["file_id"])
        return drive_main, drive_additional

    def cleanup_new_uploads(paths):
        if callable(remove_uploaded_relative_paths):
            remove_uploaded_relative_paths(paths)

    def is_confirmed_kaika_item(item_dict):
        if not item_dict or item_dict.get("scope") not in (None, "", "admin"):
            return False
        if not callable(is_kaika_inventory_item):
            return False
        try:
            return bool(is_kaika_inventory_item(item_dict))
        except Exception:
            return False

    class DuplicateItemSubmissionError(ValueError):
        pass

    def claim_item_submission(cur, token, action, item_id=None):
        token = str(token or "").strip().lower()
        if not re.fullmatch(r"[0-9a-f]{32}", token):
            raise ProductImageValidationError("フォームの有効期限が切れました。画面を再読込してください。")
        if DATABASE_URL:
            cur.execute(
                """
                CREATE TABLE IF NOT EXISTS kaika_item_submissions (
            ...
            cur.execute(
                "DELETE FROM kaika_item_submissions WHERE created_at < CURRENT_TIMESTAMP - INTERVAL '2 days'"
            )
        else:
            cur.execute(
                """
            ...
            cur.execute(
                "DELETE FROM kaika_item_submissions WHERE created_at < datetime('now', '-2 days')"
            )
        placeholder = "%s" if DATABASE_URL else "?"
        try:
            cur.execute(
                f"""
                INSERT INTO kaika_item_submissions (token, user_id, action, item_id)
                VALUES ({placeholder}, {placeholder}, {placeholder}, {placeholder})
                """,
                (token, current_user.id, action, item_id),
            )
        except Exception as exc:
            error_text = str(exc).lower()
            if "unique" in error_text or "duplicate" in error_text or "primary key" in error_text:
                raise DuplicateItemSubmissionError("このフォームは送信済みです。") from exc
            raise
    ...
    def build_company_sales_analytics_context():
        ...
        conn, cur = open_cursor()
        try:
            cur.execute("SELECT id, username, display_name, role, stripe_subscription_id, subscription_status FROM users")
            users = rows_to_dicts(cur.fetchall())
            cur.execute(
                """
                SELECT id, user_id, product_name, purchase_price, sale_price, shipping_cost, commission,
                       sale_date, purchase_date, sales_destination, sale_type, custody_location, custody_received_at
                FROM merchandise
                """
            )
        ...
        recent_sales = []
        monthly_item_counts = {}
        kaika_user_ids = {item.get("user_id") for item in items if is_kaika_fee_item(item)}

        for item in items:
            scope = classify_company_scope(item, role_map)
            sale_date = normalize_item_date(item.get("sale_date"))
            purchase_date = normalize_item_date(item.get("custody_received_at") or item.get("purchase_date")) or today
            ...
            if is_kaika_fee_item(item) and role_map.get(item.get("user_id")) == "user" and purchase_date.strftime("%Y-%m") == current_month_key:
            ...
            if sale_date:
                ...
                if is_kaika_fee_item(item) and callable(build_user_fee_components) and scope == "user":
                    try:
        ...
        subscription_revenue = 0
        for user in users:
            if user.get("role") != "user" or not is_kaika_fee_client(user, kaika_user_ids):
                continue
                ...
                if DATABASE_URL:
                    cur.execute(
                        ...
                        WHERE user_id = {placeholder}
                          AND COALESCE(custody_location, 'kaika') = 'kaika'
                          AND DATE_TRUNC('month', COALESCE(custody_received_at::date, purchase_date::date, CURRENT_DATE)) = DATE_TRUNC('month', CURRENT_DATE)
                        """,
                        (user["id"],),
                    )
                else:
                    cur.execute(
                        ...
                        WHERE user_id = {placeholder}
                          AND COALESCE(custody_location, 'kaika') = 'kaika'
                          AND strftime('%Y-%m', COALESCE(custody_received_at, purchase_date, date('now'))) = strftime('%Y-%m', 'now')
                        """,
                        (user["id"],),
                    )
    ...
    def admin_delete_client_view(id):
        ...
        try:
            ...
            if table_exists("shikiriosho"):
                ...
                cur.execute(
                    f"DELETE FROM shikiriosho WHERE sender_id = {placeholder} OR recipient_id = {placeholder}",
                    (id, id),
                )
    ...
    def edit_item_v2(id):
        ...
        try:
            ...
            lock_sql = " FOR UPDATE" if DATABASE_URL and request.method == "POST" else ""
            cur.execute(f"SELECT * FROM merchandise WHERE id = {placeholder}{lock_sql}", (id,))
            item = cur.fetchone()
            if not item:
            ...
            item_dict = dict(item)
            is_kaika_scope = is_confirmed_kaika_item(item_dict)

            if request.method == "POST":
                reject_legacy_drive_paths()
                drive_main, drive_additional = collect_verified_drive_uploads(is_kaika_scope)
                if is_kaika_scope:
                    claim_item_submission(
                        cur,
                        request.form.get("item_submission_token"),
                        "edit",
                        id,
                    )
                local_main = request.files.get("photo")
                if (
                    drive_main
                    and getattr(drive_main, "filename", "")
                    and local_main
                    and getattr(local_main, "filename", "")
                ):
                    raise ProductImageValidationError(
                        "メイン画像はPCまたはGoogle Driveのどちらか1枚を選択してください。"
                    )

                photo_path = item_dict.get("photo_path")
                uploaded_photo = save_uploaded_image(drive_main or local_main)
                if uploaded_photo:
                    photo_path = uploaded_photo
                    created_uploads.append(uploaded_photo)

                existing_additional = normalize_path_list(item_dict.get("additional_photos"))
                delete_photos = set(request.form.getlist("delete_photos"))
                additional_photos = [path for path in existing_additional if path not in delete_photos]
                new_additional_files = list(drive_additional) + request.files.getlist("additional_photos")
                new_additional_files = [
                    file_storage
                    for file_storage in new_additional_files
                    if file_storage and getattr(file_storage, "filename", "")
                ]
                if len(additional_photos) + len(new_additional_files) > 19:
                    raise ProductImageValidationError("追加画像は既存画像を含めて最大19枚です。")
                for file_storage in new_additional_files:
                    saved_path = save_uploaded_image(file_storage, prefix=f"{len(additional_photos) + 2}_")
                    if saved_path:
                        additional_photos.append(saved_path)
                        created_uploads.append(saved_path)
                ...
                if uploaded_id_document:
                    ...
                    created_uploads.append(uploaded_id_document)
                ...
                if uploaded_consent_form:
                    ...
                    created_uploads.append(uploaded_consent_form)
            ...
            traceback.print_exc()
            conn.rollback()
            cleanup_new_uploads(created_uploads)
        ...
        return render_template(
            ...
            item_submission_token=uuid.uuid4().hex if is_kaika_scope else "",
        )
    ...
    def admin_add_item_v2():
        ...
        if request.method == "POST":
            ...
            try:
                ...
                if mode == "user":
                    ...
                    target_user = cur.fetchone()
                    target_user = dict(target_user) if target_user else None
                    if not target_user or target_user.get("role") not in {"admin", "owner"}:
                        raise ProductImageValidationError("開花商品の所有条件を確認できません。")

                reject_legacy_drive_paths()
                drive_main, drive_additional = collect_verified_drive_uploads(mode == "admin")
                if mode == "admin":
                    claim_item_submission(
                        cur,
                        request.form.get("item_submission_token"),
                ...
                photo_path = save_uploaded_image(drive_main or local_main)
                if photo_path:
                    created_uploads.append(photo_path)

                additional_files = list(drive_additional) + request.files.getlist("additional_photos")
                additional_files = [
                    file_storage
                    for file_storage in additional_files
                    if file_storage and getattr(file_storage, "filename", "")
                ]
                if len(additional_files) > 19:
                    raise ProductImageValidationError("追加画像は最大19枚です。")
                additional_photos = []
                for file_storage in additional_files:
                    saved_path = save_uploaded_image(file_storage, prefix=f"{len(additional_photos) + 2}_")
                    if saved_path:
                ...
                if id_document_path:
                    created_uploads.append(id_document_path)
                if consent_form_path:
                    created_uploads.append(consent_form_path)
        ...
        return render_template(
            ...
            item_submission_token=uuid.uuid4().hex if mode == "admin" else "",
        )
